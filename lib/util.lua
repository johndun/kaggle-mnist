...
function random_jitter(src)
  local out_sz = 24
  local crp_sz = math.random(24, 32)
  local crp_off_x = math.random(1, 32 - crp_sz + 1)
  local crp_off_y = math.random(1, 32 - crp_sz + 1)
...
-- Single image ittering for augmented testing
function test_jitter(src)
  local out_sz = 24
  local out = torch.Tensor(18, src:size(1), out_sz, out_sz)
  local i = 1
  for x_off = 0, 4, 2 do
    for y_off = 0, 4, 2 do
      local new_img = sample_image{src       = src,
                                   crp_off_x = x_off, 
                                   crp_off_y = y_off, 
                                   crp_sz_x  = 28, 
                                   crp_sz_y  = 28, 
